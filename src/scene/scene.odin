...
basic_scene :: proc() -> Scene {
	scene := scene_with_camera(
		scene_camera({0, 1, 4}, {0, 0, -1.5}, 45, 0, 5, 25, 10),
		5,
	)
	append_ground(&scene)
...
materials_scene :: proc() -> Scene {
	scene := scene_with_camera(
		scene_camera({0, 1, 5}, {0, 0, -3}, 40, 0, 6, 50, 20),
		4,
	)
	append_ground(&scene)
	append(&scene.objects, SceneObject {
...
defocus_scene :: proc() -> Scene {
	scene := scene_with_camera(
		scene_camera({0, 1.5, 4}, {0, 0, -3}, 40, 6, 5, 50, 20),
		5,
	)
	append_ground(&scene)
	append(&scene.objects, SceneObject {
...
random_scene :: proc(grid_radius: int = 5) -> Scene {
	...
	scene := scene_with_camera(
		scene_camera({-2, 2, 1}, {0, 0, -1.5}, 50, 1, 3.4, 100, 50),
		capacity,
	)
	append_ground(&scene)
...
random_sphere :: proc(i, j: int) -> SceneObject {
	...
	case 0:
		material = SceneMaterial{kind = .lambertian, albedo = random_color()}
	case 1:
		material = SceneMaterial{kind = .metal, albedo = random_color(), fuzz = rand.float64()}
	case:
		material = SceneMaterial{kind = .dielectric, refraction_index = rand.float64()}
	}
